...
var promise = {
	...
	vouch : function ( state, fn ) {
		if ( string.isEmpty( state ) ) throw Error( label.error.invalidArguments );
...
var xhr = function () {
	...
	headers = {
		"User-Agent"   : "abaaso/3.5.12 node.js/" + process.versions.node.replace( /^v/, "" ) + " (" + string.capitalize( process.platform ) + " V8/" + process.versions.v8 + " )",
