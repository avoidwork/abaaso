		...
		event.each(function (i) {
			...
			if (n) {
				...
				if (instance !== null && typeof instance !== "undefined" && i.toLowerCase() !== "afterjsonp" && (globals.test(o) || typeof instance.listeners === "function")) {
					...
					if (reg) instance[add ? "addEventListener" : "attachEvent"]((add ? "" : "on") + i, handler, false);
				}
			}
...
var route = {
	...
	server : function (args, fn, ssl) {
		...
		handler = function (req, res) {
			...
			route.load(parsed.pathname, res, req, parsed.hostname || "all");
		};
		...
		// Creating server
		obj = !ssl ? http.createServer(handler).on("error", err).listen(args.port, args.host) : https.createServer(args, handler).on("error", err).listen(args.port);
...
var xhr = function () {
	...
	headers = {
		"User-Agent"   : "abaaso/2.9.13 node.js/" + process.versions.node.replace(/^v/, "") + " (" + string.capitalize(process.platform) + " V8/" + process.versions.v8 + ")",
