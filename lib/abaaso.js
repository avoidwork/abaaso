...
var xhr = function () {
	...
	headers = {
		"User-Agent"   : "abaaso/3.5.10 node.js/" + process.versions.node.replace(/^v/, "") + " (" + string.capitalize(process.platform) + " V8/" + process.versions.v8 + ")",
	...
	if (!server) {
		...
		$.on(global, "hashchange", function (e)  {
			...
			if ($.route.current !== hash || self.route.current !== hash) {
				...
				if ($.route.current !== self.route.current) $.route.current = self.route.current; // IE8 specific
