...
if (server && typeof XMLHttpRequest === "undefined") XMLHttpRequest = require("xmlhttprequest").XMLHttpRequest;
	...
	data = {
		...
		methods : {
			...
			storage : function (obj, op, type) {
				...
				if (typeof Storage === "undefined" || !/number|object|string/.test(typeof obj) || !/get|remove|set/.test(op)) throw Error(label.error.invalidArguments);

				record = (ns.test(obj) || (obj.hasOwnProperty("key") && !obj.hasOwnProperty("parentNode")));

				switch (op) {
					case "get":
						result = session ? sessionStorage.getItem(obj) : localStorage.getItem(obj);
						if (result === null) throw Error(label.error.invalidArguments);
						result = json.decode(result);
						if (record) this.get((ns.test(obj) ? obj : obj.key)).data = result;
						else this.data = result;
						result = ns.test(obj) ? this.get(obj) : this.data;
						break;
					case "remove":
						session ? sessionStorage.removeItem(obj) : localStorage.removeItem(obj);
						result = obj;
						break;
					case "set":
						key  = record ? obj.key : obj.parentNode.id;
						data = json.encode(record ? obj.data : obj.get());
