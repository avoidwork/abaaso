...
abaaso = global.abaaso || (function () {
	...
	element = {
		...
		hasClass : function (obj, klass) {
			obj = utility.object(obj);
			if (obj instanceof Array) return obj.each(function (i) { element.hasClass(i, klass); });
	...
	bootstrap = function () {
		...
		if (!server && typeof document.documentElement.classList === "undefined") {
			document.documentElement.classList = {
				add : function (arg) {
					var classes = array.add(string.explode(this.className, " "), arg).join(" ").trim();

					!client.ie || client.version > 8 ? this.attr("class", classes) : this.className = classes;
					return this;
				},
				contains : function (arg) {
					return string.explode(this.className, " ").index(arg) > -1;
				},
				remove : function (arg) {
					var classes = array.remove(string.explode(this.className, " "), arg).join(" ").trim();

					!client.ie || client.version > 8 ? this.attr("class", classes) : this.className = classes;
					return this;
				},
				toggle : function (arg) {
					this.classList[this.classList.contains(arg) ? "remove" : "add"](arg);
					return this;
				}
			};
		};
