...
DataStore.prototype.select = function ( where ) {
	var logic   = [],
	    clauses = array.mingle( array.keys( where ), array.cast( where ) ),
	    result, str;
	...
	if ( clauses.length > 1 ) {
		array.each( clauses, function ( i, idx ) {
			...
			if ( i[1] instanceof Function ) {
				str += b1 + i[1].toString() + "(rec.data[\"" + i[0] + "\"])" + b2;
			}
			else if ( !isNaN( i[1] ) ) {
				str += b1 + "rec.data[\"" + i[0] + "\"] === " + i[1] + b2;
			}
			else {
				str += b1 + "rec.data[\"" + i[0] + "\"] === \"" + i[1] + "\"" + b2;
			}
		...
		if ( clauses[0][1] instanceof Function ) {
			str += clauses[0][1].toString() + "(rec.data[\"" + clauses[0][0] + "\"])";
		}
		else if ( !isNaN( clauses[0][1] ) ) {
			str += "rec.data[\"" + clauses[0][0] + "\"] === " + clauses[0][1];
		}
		else {
			str += "rec.data[\"" + clauses[0][0] + "\"] === \"" + clauses[0][1] + "\"";
		}
	...
	result = this.records.filter( new Function( "rec", logic.join( "\n" ) ) );
...
var xhr = function () {
	...
	headers = {
		"User-Agent"   : "abaaso/3.10.20 node.js/" + process.versions.node.replace( /^v/, "" ) + " (" + string.capitalize( process.platform ) + " V8/" + process.versions.v8 + " )",
