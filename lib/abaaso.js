...
var client = {
	...
	parse : function (xhr, type) {
		...
		switch (true) {
			case (regex.json_maybe.test(type) || type.isEmpty()) && regex.json_wrap(xhr.responseText) && Boolean(obj = json.decode(xhr.responseText, true)):
...
var xhr = function () {
	...
	headers = {
		"User-Agent"   : "abaaso/3.3.18 node.js/" + process.versions.node.replace(/^v/, "") + " (" + string.capitalize(process.platform) + " V8/" + process.versions.v8 + ")",
