...
number_format_2:/\..*/,number_present:/\d{1,}/,number_string:/number|string/i,number_string_object:/number|object|string/i,null_undefined:/null|undefined/,observer_allowed:/click|error|key|mousedown|mouseup|touchstart|touchend|touchmove|touchenter|touchleave|touchcancel|submit/i,observer_globals:/body|document|window/i,object_type:/\[object Object\]/,object_undefined:/object|undefined/,opera:/opera/i,osx:/macintosh/i,patch:/^patch$/,phone:/^([0-9\(\)\/\+ \-\.]+)$/,playbook:/playbook/i,plural:/s$/,
primitive:/^(boolean|function|number|string)$/,priv:/private/,put_post:/^(post|put)$/i,radio_checkbox:/^(radio|checkbox)$/i,reflect:/function\s+\w*\s*\((.*?)\)/,root:/^\/[^\/]/,route_nget:/^(head|options)$/i,route_methods:/^(all|delete|get|put|post|head|options)$/i,safari:/safari/i,scheme:/.*\/\//,select:/select/i,selector_is:/^:/,selector_many:/\:|\.|\+|\~|\[/,selector_complex:/\s+|\>|\+|\~|\:|\[/,selector_split:/\s+|\>|\+|\~/,set_del:/^(set|del|delete)$/,sort_needle:/^.*:::/,sort_value:/:::.*$/,
...
else return Math.ceil(arg)}};var observer={elisteners:{},ignore:false,listeners:{},log:false,maxListeners:10,queue:[],silent:false,add:function(obj,events,fn,id,scope,st){var oId=observer.id(obj),instance=regex.observer_globals.test(oId)||!/\//g.test(oId)&&oId!=="abaaso"?obj:null;id=id||utility.genId();scope=scope||obj;st=st||state.getCurrent();if(!observer.listeners[oId]){observer.listeners[oId]={};observer.listeners[oId].all={}}if(st!=="all"&&!observer.listeners[oId][st])observer.listeners[oId][st]=
{};array.each(array.explode(events),function(ev){var eId=oId+"_"+ev,add,reg;if(!observer.listeners[oId].all[ev])observer.listeners[oId].all[ev]=lru(observer.maxListeners);if(st!=="all"&&!observer.listeners[oId][st][ev])observer.listeners[oId][st][ev]=lru(observer.maxListeners);if(instance&&(ev.toLowerCase()!=="afterjsonp"&&(!observer.elisteners[eId]&&regex.observer_globals.test(oId)))){add=typeof instance.addEventListener==="function";reg=typeof instance.attachEvent==="object"||add;if(reg){observer.elisteners[eid]=
function(e){if(!regex.observer_allowed.test(e.type))utility.stop(e);observer.fire(oId,ev,e)};instance[add?"addEventListener":"attachEvent"]((add?"":"on")+ev,observer.elisteners[eId],false)}}observer.listeners[oId][st].set(id,{fn:fn,scope:scope})})},decorate:function(){var methods=[["fire",function(){return observer.fire.apply(observer,[this].concat(array.cast(arguments)))}],["listeners",function(e){return observer.list(this,e)}],["on",function(e,listener,id,scope,standby){return observer.add(this,
...
else if(observer.listeners[oId]){oId=observer.id(obj);args=array.remove(array.cast(arguments),0,1);log=abaaso.logging;array.each(string.explode(events),function(ev){if(log)utility.log(oId+" fired "+ev);array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&observer.listeners[oId][st][ev]){cache=observer.listeners[oId][st][ev];item=cache.get(cache.last);do if(item.fn.apply(item.scope,args)!==false)item=cache.get(item.previous);else return false;while(item)}})})}return obj},id:function(arg){var id;
if(arg===global)id="window";else if(!server&&arg===document)id="document";else if(!server&&arg===document.body)id="body";else{utility.genId(arg);id=arg.id||(typeof arg.toString==="function"?arg.toString():arg)}return id},list:function(obj,ev){var oId=observer.id(obj),states=observer.states(),result={};if(!observer.listeners[oId]);else if(!ev||string.isEmpty(ev))array.each(observer.states(),function(st){if(observer.listeners[oId][st])array.each(array.keys(observer.listeners[oId][st]),function(ev){result[st]=
observer.listeners[oId][st][ev].cache})});else array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&observer.listeners[oId][st][ev])result[st]=observer.listeners[oId][st][ev].cache});return result},once:function(obj,events,fn,id,scope,st){id=id||utility.genId();scope=scope||obj;st=st||state.getCurrent();array.each(array.explode(events),function(ev){observer.add(obj,ev,function(){fn.apply(scope,arguments);observer.remove(obj,ev,id,st)},id,scope,st)});return obj},pause:function(arg){if(arg===
true)observer.silent=arg;else if(arg===false){observer.silent=arg;array.each(observer.queue,function(i){observer.fire.apply(observer,i)});observer.queue=[]}return observer.silent},remove:function(obj,event,id,st){var oId=observer.id(obj),add=typeof obj.addEventListener==="function",reg=typeof obj.attachEvent==="object"||add,fn;st=st||state.getCurrent();fn=function(event,i){var unhook=typeof i==="number"&&(cl[o][event]=cl[o][event]-i)===0;if(unhook&&reg){obj[add?"removeEventListener":"detachEvent"]((add?
"":"on")+event,ev[o+"_"+event],false);delete ev[o+"_"+event]}};if(!observer.listeners[oId])return obj;if(event){if(regex.observer_globals.test(o)||typeof o.listeners==="function")utility.iterate(ev,function(v,k){if(k.indexOf(o+"_")===0)fn(k.replace(/.*_/,""),1)});delete l[o];delete a[o];delete cl[o]}else array.each(string.explode(event),function(e){var sync=false;if(l[o][e]===undefined)return;if(id===undefined){if(regex.observer_globals.test(o)||typeof o.listeners==="function")fn(e,array.keys(l[o][e][st]).length);
l[o][e][st]={};sync=true}else if(l[o][e][st][id]!==undefined){fn(e,1);delete l[o][e][st][id];sync=true}if(sync)observer.sync(o,e,st)});return obj},states:function(){return["all",state.getCurrent()]}};var promise={delay:function(){if(typeof setImmediate!=="undefined")return setImmediate;else if(typeof process!=="undefined")return process.nextTick;else return function(arg){setTimeout(arg,0)}}(),factory:function(){return new Promise},pipe:function(parent,child){parent.then(function(arg){child.resolve(arg)},
