...
recs);return obj},worker:function(ev){var cmd=ev.data.cmd,logic,result,where;if(cmd==="select"){logic=[];where=JSON.parse(ev.data.where);utility.iterate(where,function(v,k){if(array.contains(ev.data.functions,k))logic.push("if ("+v+'(rec.data["'+k+'"])) return true;');else if(!isNaN(v))logic.push('if (rec.data["'+k+'"] === '+v+") return true;");else logic.push('if (rec.data["'+k+'"] === "'+v+'") return true;')});logic.push("return false;");result=ev.data.records.filter(new Function("rec",logic.join("\n")))}else if(cmd===
"sort")result=array.keySort(ev.data.records,ev.data.query,"data");postMessage(result)}};function DataStore(obj){this.autosave=false;this.callback=null;this.collections=[];this.credentials=null;this.datalists=[];this.depth=0;this.events=false;this.expires=null;this.headers={Accept:"application/json"};this.id=utility.uuid();this.ignore=[];this.key=null;this.keys={};this.leafs=[];this.loaded=false;this.maxDepth=0;this.mongodb="";this.parentNode=obj;this.pointer=null;this.records=[];this.retrieve=false;
this.source=null;this.total=0;this.views={};this.uri=null}DataStore.prototype.constructor=DataStore;DataStore.prototype.batch=function(type,data,sync){if(!regex.set_del.test(type)||(sync&&regex.del.test(type)||typeof data!=="object"))throw new Error(label.error.invalidArguments);sync=sync===true;var self=this,events=this.events,defer=deferred(),deferreds=[];if(events)observer.fire(self.parentNode,"beforeDataBatch",data);if(sync)this.clear(sync);if(data.length===0)defer.resolve(this.records);else{if(type===
...
DataStore.prototype.purge=function(arg){return this.storage(arg||this,"remove")};DataStore.prototype.reindex=function(){var nth=this.total,i=-1;this.views={};if(nth>0)while(++i<nth){this.records[i].index=i;this.keys[this.records[i].key]=i}return this};DataStore.prototype.restore=function(arg){return this.storage(arg||this,"get")};DataStore.prototype.save=function(arg){return this.storage(arg||this,"set")};DataStore.prototype.select=function(where){var defer=deferred(),blob,functions,logic,worker;
if(!(where instanceof Object))throw new Error(label.error.invalidArguments);if(server){logic=[];utility.iterate(where,function(v,k){if(v instanceof Function)logic.push("if ("+v.toString()+'(rec.data["'+k+'"])) return true;');else if(!isNaN(v))logic.push('if (rec.data["'+k+'"] === '+v+") return true;");else logic.push('if (rec.data["'+k+'"] === "'+v+'") return true;')});logic.push("return false;");defer.resolve(this.records.slice().filter(new Function("rec",logic.join("\n"))))}else{functions=[];utility.iterate(where,
function(v,k){if(typeof v==="function"){this[k]=v.toString();functions.push(k)}});blob=new Blob([WORKER]);worker=new Worker(global.URL.createObjectURL(blob));worker.onmessage=function(ev){defer.resolve(ev.data)};worker.postMessage({cmd:"select",records:this.records,where:json.encode(where),functions:functions})}return defer};DataStore.prototype.set=function(key,data,batch){data=utility.clone(data,true);batch=batch===true;var self=this,events=this.events,defer=deferred(),record=key!==null?this.get(key)||
null:null,method="POST",parsed=utility.parse(self.uri||""),uri;if(typeof data==="string"){if(data.indexOf("//")===-1)if(data.charAt(0)!=="/")uri=this.buildUri(data);else if(self.uri!==null&&regex.root.test(data))uri=parsed.protocol+"//"+parsed.host+data;else uri=data;else uri=data;key=uri.replace(regex.not_endpoint,"");if(string.isEmpty(key))defer.reject(new Error(label.error.invalidArguments));else{if(!batch&&events)observer.fire(self.parentNode,"beforeDataSet",{key:key,data:data});client.request(uri,
"GET",function(arg){self.setComplete(record,key,self.source?arg[self.source]:arg,batch,defer)},function(e){observer.fire(self.parentNode,"failedDataSet",e);defer.reject(e)},undefined,utility.merge({withCredentials:self.credentials},self.headers))}}else{if(record===null&&(key===null||key===undefined))if(this.key===null)key=utility.uuid();else if(data[this.key]){key=data[this.key];delete data[this.key]}else key=utility.uuid();if(!batch&&events)observer.fire(self.parentNode,"beforeDataSet",{key:key,
data:data});if(batch||this.uri===null)this.setComplete(record,key,data,batch,defer);else{if(key!==null){method="PUT";uri=this.buildUri(key);if(client.allows(uri,"patch")&&(!client.ie||client.activex))method="PATCH";else if(record!==null)utility.iterate(record.data,function(v,k){if(!array.contains(self.collections,k)&&!data[k])data[k]=v})}else uri=this.uri;client.request(uri,method,function(arg){self.setComplete(record,key,self.source?arg[self.source]:arg,batch,defer)},function(e){observer.fire(self.parentNode,
"failedDataSet",e);defer.reject(e)},data,utility.merge({withCredentials:this.credentials},this.headers))}}return defer};DataStore.prototype.setComplete=function(record,key,data,batch,defer){var self=this,deferreds=[];if(record===null){record={index:this.total++,key:key,data:data};this.keys[key]=record.index;this.records[record.index]=record;if(this.retrieve)deferreds.push(this.crawl(record))}else utility.iterate(data,function(v,k){if(!array.contains(self.collections,k))record.data[k]=v;else if(typeof v===
...
return obj},create:function(type,args,target,pos){var svg=false,frag=false,obj,uid,result;type=type.replace(/\t|\n|\r/g,"");if(target!==undefined)svg=target.namespaceURI!==undefined&&regex.svg.test(target.namespaceURI);else target=document.body;if(args instanceof Object&&(args.id!==undefined&&utility.dom("#"+args.id)===undefined)){uid=args.id;delete args.id}else if(!svg)uid=utility.genId(undefined,true);if(regex.html.test(type)){frag=true;obj=element.frag(type);result=obj.childNodes.length===1?obj.childNodes[0]:
array.cast(obj.childNodes)}else{if(!svg&&!regex.svg.test(type))obj=document.createElement(type);else obj=document.createElementNS("http://www.w3.org/2000/svg",type);if(uid!==undefined)obj.id=uid;if(args instanceof Object)element.update(obj,args)}if(pos===undefined||pos==="last")target.appendChild(obj);else if(pos==="first")element.prependChild(target,obj);else if(pos==="after"){pos={};pos.after=target;target=target.parentNode;target.insertBefore(obj,pos.after.nextSibling)}else if(pos.after!==undefined)target.insertBefore(obj,
pos.after.nextSibling);else if(pos==="before"){pos={};pos.before=target;target=target.parentNode;target.insertBefore(obj,pos.before)}else if(pos.before!==undefined)target.insertBefore(obj,pos.before);else target.appendChild(obj);return!frag?obj:result},css:function(obj,key,value){key=string.toCamelCase(key);if(value!==undefined){obj.style[key]=value;return obj}else return obj.style[key]},data:function(obj,key,value){if(value!==undefined){obj.setAttribute("data-"+key,regex.json_wrap.test(value)?json.encode(value):
...
width;bottom=height}return[left,top,right,bottom]},prependChild:function(obj,child){return obj.childNodes.length===0?obj.appendChild(child):obj.insertBefore(child,obj.childNodes[0])},removeAttr:function(obj,key){var target;if(regex.svg.test(obj.namespaceURI))obj.removeAttributeNS(obj.namespaceURI,key);else if(obj.nodeName==="SELECT"&&key==="selected"){target=utility.dom("#"+obj.id+' option[selected="selected"]')[0];if(target!==undefined){target.selected=false;target.removeAttribute("selected")}}else obj.removeAttribute(key);
return obj},scrollTo:function(obj,ms,offsetTop,offsetLeft){var pos=array.remove(element.position(obj),2,3);if(!isNaN(offsetTop))pos[0]+=offsetTop;if(!isNaN(offsetLeft))pos[1]+=offsetLeft;return client.scroll(pos,ms)},serialize:function(obj,string,encode){string=string===true;encode=encode!==false;var children=[],registry={},result;children=obj.nodeName==="FORM"?obj.elements!==undefined?array.cast(obj.elements):obj.find("button, input, select, textarea"):[obj];array.each(children,function(i){if(i.nodeName===
"FORM")utility.merge(registry,json.decode(element.serialize(i)));else if(registry[i.name]===undefined)registry[i.name]=element.val(i)});if(!string)result=json.encode(registry);else{result="";utility.iterate(registry,function(v,k){encode?result+="&"+encodeURIComponent(k)+"="+encodeURIComponent(v):result+="&"+k+"="+v});result=result.replace(regex.and,"?")}return result},size:function(obj){return{height:obj.offsetHeight+number.parse(obj.style.paddingTop||0)+number.parse(obj.style.paddingBottom||0)+number.parse(obj.style.borderTop||
0)+number.parse(obj.style.borderBottom||0),width:obj.offsetWidth+number.parse(obj.style.paddingLeft||0)+number.parse(obj.style.paddingRight||0)+number.parse(obj.style.borderLeft||0)+number.parse(obj.style.borderRight||0)}},text:function(obj,arg){var key=obj.textContent!==undefined?"textContent":"innerText",payload={},set=false;if(typeof arg!=="undefined"){set=true;payload[key]=arg}return set?element.update(obj,payload):obj[key]},toggleClass:function(obj,arg){obj.classList.toggle(arg);return obj},
update:function(obj,args){args=args||{};utility.iterate(args,function(v,k){if(regex.element_update.test(k))obj[k]=v;else if(k==="class")!string.isEmpty(v)?element.klass(obj,v):element.klass(obj,"*",false);else if(k.indexOf("data-")===0)element.data(obj,k.replace("data-",""),v);else if(k==="id"){var o=observer.listeners;if(o[obj.id]!==undefined){o[k]=o[obj.id];delete o[obj.id]}}else element.attr(obj,k,v)});return obj},val:function(obj,value){var event="input",output;if(value===undefined){if(regex.radio_checkbox.test(obj.type)){if(string.isEmpty(obj.name))throw new Error(label.error.expectedProperty);
