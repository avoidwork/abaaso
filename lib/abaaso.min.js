...
else return Math.ceil(arg)}};var observer={elisteners:{},ignore:false,listeners:{},log:false,maxListeners:10,queue:[],silent:false,add:function(obj,events,fn,id,scope,st){var oId=observer.id(obj),instance=regex.observer_globals.test(oId)||!/\//g.test(oId)&&oId!=="abaaso"?obj:null;if(!oId||(!events||typeof fn!=="function"))throw new Error(label.error.invalidArguments);id=id||utility.uuid();scope=scope||obj;st=st||state.getCurrent();if(!observer.listeners[oId]){observer.listeners[oId]={};observer.listeners[oId].all=
{}}if(st!=="all"&&!observer.listeners[oId][st])observer.listeners[oId][st]={};array.each(string.explode(events),function(ev){var eId=oId+"_"+ev,add,reg;if(!observer.listeners[oId].all[ev])observer.listeners[oId].all[ev]=lru(observer.maxListeners);if(st!=="all"&&!observer.listeners[oId][st][ev])observer.listeners[oId][st][ev]=lru(observer.maxListeners);if(instance&&(ev.toLowerCase()!=="afterjsonp"&&(!observer.elisteners[eId]&&(regex.observer_globals.test(oId)||obj.nodeName)))){add=typeof instance.addEventListener===
"function";reg=typeof instance.attachEvent==="object"||add;if(reg){observer.elisteners[eId]=function(e){observer.fire(oId,ev,e)};instance[add?"addEventListener":"attachEvent"]((add?"":"on")+ev,observer.elisteners[eId],false)}}observer.listeners[oId][st][ev].set(id,{fn:fn,scope:scope})});return obj},decorate:function(){var methods=[["fire",function(){return observer.fire.apply(observer,[this].concat(array.cast(arguments)))}],["listeners",function(e){return observer.list(this,e)}],["on",function(e,
listener,id,scope,standby){return observer.add(this,e,listener,id,scope,standby)}],["once",function(e,listener,id,scope,standby){return observer.once(this,e,listener,id,scope,standby)}],["un",function(e,id){return observer.remove(this,e,id)}]];array.each(methods,function(i){utility.property(obj,i[0],{value:i[1],configurable:true,enumerable:true,writable:true})});return obj},discard:function(arg){return arg===undefined?observer.ignore:observer.ignore=arg===true},fire:function(obj,events){var args,
log,cache,item,oId;if(!observer.ignore)if(observer.silent)observer.queue.push(array.cast(arguments));else{oId=observer.id(obj);if(observer.listeners[oId]){args=array.remove(array.cast(arguments),0,1);log=abaaso.logging;array.each(string.explode(events),function(ev){if(log)utility.log(oId+" fired "+ev);array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&observer.listeners[oId][st][ev]){cache=observer.listeners[oId][st][ev];if(cache.length>0){item=cache.cache[cache.last];do if(item.value.fn.apply(item.value.scope,
args)!==false&&item.next)item=cache.cache[item.next];else return false;while(item)}}})})}}return obj},id:function(arg){var id;if(arg===global)id="window";else if(!server&&arg===document)id="document";else if(!server&&arg===document.body)id="body";else{utility.genId(arg);id=arg.id||(typeof arg.toString==="function"?arg.toString():arg)}return id},list:function(obj,ev){var oId=observer.id(obj),states=observer.states(),result={};if(!observer.listeners[oId]);else if(!ev||string.isEmpty(ev))array.each(observer.states(),
function(st){if(observer.listeners[oId][st])array.each(array.keys(observer.listeners[oId][st]),function(ev){result[st]=observer.listeners[oId][st][ev].cache})});else array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&observer.listeners[oId][st][ev])result[st]=observer.listeners[oId][st][ev].cache});return result},once:function(obj,events,fn,id,scope,st){id=id||utility.uuid();scope=scope||obj;st=st||state.getCurrent();if(!obj||(!events||typeof fn!=="function"))throw new Error(label.error.invalidArguments);
...
"object"||add,regex=/.*_/,states,total,unhook;st=st||state.getCurrent();unhook=function(eId,ev){if(reg)obj[add?"removeEventListener":"detachEvent"]((add?"":"on")+ev,this[eId],false);delete this[eId]};if(observer.listeners[oId])if(!events){utility.iterate(observer.elisteners,function(v,k){if(k.indexOf(oId+"_")===0)unhook.call(this,k,k.replace(regex,""))});delete observer.listeners[oId]}else{events=string.explode(events);states=array.keys(observer.listeners[oId]);total=0;array.each(states,function(s){array.each(events,
function(e){if(observer.listeners[oId][s][e])total+=observer.listeners[oId][s][e].length})});array.each(events,function(ev){if(observer.listeners[oId][st][ev])if(id){observer.listeners[oId][st][ev].remove(id);total--;if(total===0){array.each(states,function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}}else{array.each(states,function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}})}return obj},states:function(){return["all",
state.getCurrent()]}};var promise={delay:function(){if(typeof setImmediate!=="undefined")return setImmediate;else if(typeof process!=="undefined")return process.nextTick;else return function(arg){setTimeout(arg,0)}}(),factory:function(){return new Promise},pipe:function(parent,child){parent.then(function(arg){child.resolve(arg)},function(e){child.reject(e)})},state:{PENDING:0,FAILURE:1,SUCCESS:2}};function Promise(){this.deferred=false;this.handlers=[];this.state=promise.state.PENDING;this.value=
null}Promise.prototype.constructor=Promise;Promise.prototype.process=function(){var result,success,value;this.deferred=false;if(this.state===promise.state.PENDING)return;value=this.value;success=this.state===promise.state.SUCCESS;array.each(this.handlers.slice(),function(i){var callback=i[success?"success":"failure"],child=i.promise;if(!callback||typeof callback!=="function"){if(value&&typeof value.then==="function")promise.pipe(value,child);else if(success)child.resolve(value);else child.reject(value);
return}try{result=callback(value)}catch(e){child.reject(e);return}if(result&&typeof result.then==="function")promise.pipe(result,promise);else child.resolve(result)});return this};Promise.prototype.reject=function(arg){var self=this;if(this.state>promise.state.PENDING)return;this.value=arg;this.state=promise.state.FAILURE;if(!this.deferred){promise.delay(function(){self.process()});this.deferred=true}return this};Promise.prototype.resolve=function(arg){var self=this;if(this.state>promise.state.PENDING)return;
this.value=arg;this.state=promise.state.SUCCESS;if(!this.deferred){promise.delay(function(){self.process()});this.deferred=true}return this};Promise.prototype.then=function(success,failure){var self=this,child=new Promise;this.handlers.push({success:success,failure:failure,promise:child});if(this.state>promise.state.PENDING&&!this.deferred){promise.delay(function(){self.process()});this.deferred=true}return child};var prototypes={array:{add:function(arg){return array.add(this,arg)},addClass:function(arg){return array.each(this,
function(i){element.klass(i,arg)})},after:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,"after"))});return result},append:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,"last"))});return result},attr:function(key,value){var result=[];array.each(this,function(i){result.push(element.attr(i,key,value))});return result},before:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,
args,i,"before"))});return result},binIndex:function(arg){return array.binIndex(this,arg)},chunk:function(size){return array.chunk(this,size)},clear:function(){return!server&&this[0]instanceof Element?array.each(this,function(i){element.clear(i)}):array.clear(this)},clone:function(){return utility.clone(this)},collect:function(arg){return array.collect(this,arg)},compact:function(){return array.compact(this)},contains:function(arg){return array.contains(this,arg)},count:function(arg){return array.count(this,
arg)},create:function(type,args,position){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,position))});return result},css:function(key,value){return array.each(this,function(i){element.css(i,key,value)})},data:function(key,value){var result=[];array.each(this,function(i){result.push(element.data(i,key,value))});return result},diff:function(arg){return array.diff(this,arg)},disable:function(){return array.each(this,function(i){element.disable(i)})},dispatch:function(event,
data,bubbles,cancelable){return array.each(this,function(i){element.dispatch(i,event,data,bubbles,cancelable)})},destroy:function(){array.each(this,function(i){element.destroy(i)});return[]},each:function(arg,async,size){return array.each(this,arg,async,size)},empty:function(){return array.empty(this)},enable:function(){return array.each(this,function(i){element.enable(i)})},equal:function(arg){return array.equal(this,arg)},fib:function(arg){return array.fib(arg)},fill:function(arg,start,offset){return array.fill(this,
arg,start,offset)},find:function(arg){var result=[];array.each(this,function(i){element.find(i,arg).each(function(r){result.add(r)})});return result},fire:function(){var args=arguments;return array.each(this,function(i){observer.fire.apply(observer,[i].concat(array.cast(args)))})},first:function(){return array.first(this)},flat:function(){return array.flat(this)},genId:function(){return array.each(this,function(i){utility.genId(i)})},get:function(uri,headers){var result=[];array.each(this,function(i,
idx){i.get(uri,headers,function(arg){result[idx]=arg},function(e){result[idx]=e})});return result},has:function(arg){var result=[];array.each(this,function(i){result.push(element.has(i,arg))});return result},hasClass:function(arg){var result=[];array.each(this,function(i){result.push(element.hasClass(i,arg))});return result},html:function(arg){var result;if(arg!==undefined)return array.each(this,function(i){element.html(i,arg)});else{result=[];array.each(this,function(i){result.push(element.html(i))});
return result}},index:function(arg){return array.index(this,arg)},indexed:function(){return array.indexed(this)},intersect:function(arg){return array.intersect(this,arg)},is:function(arg){var result=[];array.each(this,function(i){result.push(element.is(i,arg))});return result},isAlphaNum:function(){var result=[];array.each(this,function(i){result.push(i.isAlphaNum())});return result},isBoolean:function(){var result=[];array.each(this,function(i){result.push(i.isBoolean())});return result},isChecked:function(){var result=
[];array.each(this,function(i){result.push(i.isChecked())});return result},isDate:function(){var result=[];array.each(this,function(i){result.push(i.isDate())});return result},isDisabled:function(){var result=[];array.each(this,function(i){result.push(element.isDisabled(i))});return result},isDomain:function(){var result=[];array.each(this,function(i){result.push(i.isDomain())});return result},isEmail:function(){var result=[];array.each(this,function(i){result.push(i.isEmail())});return result},isEmpty:function(){var result=
[];array.each(this,function(i){result.push(i.isEmpty())});return result},isHidden:function(){var result=[];array.each(this,function(i){result.push(element.isHidden(i))});return result},isIP:function(){var result=[];array.each(this,function(i){result.push(i.isIP())});return result},isInt:function(){var result=[];array.each(this,function(i){result.push(i.isInt())});return result},isNumber:function(){var result=[];array.each(this,function(i){result.push(i.isNumber())});return result},isPhone:function(){var result=
[];array.each(this,function(i){result.push(i.isPhone())});return result},isUrl:function(){var result=[];array.each(this,function(i){result.push(i.isUrl())});return result},keepIf:function(fn){return array.keepIf(this,fn)},keySort:function(query,sub){return array.keySort(this,query,sub)},keys:function(){return array.keys(this)},last:function(arg){return array.last(this,arg)},limit:function(start,offset){return array.limit(this,start,offset)},listeners:function(event){var result=[];array.each(this,
function(i){array.merge(result,observer.listeners(i,event))});return result},loading:function(){return array.each(this,function(i){utility.loading(i)})},max:function(){return array.max(this)},mean:function(){return array.mean(this)},median:function(){return array.median(this)},merge:function(arg){return array.merge(this,arg)},min:function(){return array.min(this)},mingle:function(arg){return array.mingle(this,arg)},mode:function(){return array.mode(this)},on:function(event,listener,id,scope,state){return array.each(this,
function(i){observer.add(i,event,listener,id,scope||i,state)})},once:function(event,listener,id,scope,state){return array.each(this,function(i){observer.once(i,event,listener,id,scope||i,state)})},percents:function(precision,total){return array.percents(this,precision,total)},position:function(){var result=[];array.each(this,function(i){result.push(element.position(i))});return result},prepend:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,"first"))});
return result},range:function(){return array.range(this)},rassoc:function(arg){return array.rassoc(this,arg)},reject:function(fn){return array.reject(this,fn)},remove:function(start,end){return array.remove(this,start,end)},removeIf:function(fn){return array.removeIf(this,fn)},removeWhile:function(fn){return array.removeWhile(this,fn)},removeAttr:function(key){array.each(this,function(i){element.removeAttr(i,key)});return this},removeClass:function(arg){return array.each(this,function(i){element.klass(i,
arg,false)})},replace:function(arg){return array.replace(this,arg)},rest:function(arg){return array.rest(this,arg)},rindex:function(arg){return array.rindex(this,arg)},rotate:function(arg){return array.rotate(this,arg)},serialize:function(string,encode){return element.serialize(this,string,encode)},series:function(start,end,offset){return array.series(start,end,offset)},size:function(){var result=[];array.each(this,function(i){result.push(element.size(i))});return result},sorted:function(){return array.sorted(this)},
split:function(size){return array.split(this,size)},sum:function(){return array.sum(this)},take:function(arg){return array.take(this,arg)},text:function(arg){return array.each(this,function(node){if(typeof node!=="object")node=utility.object(node);if(typeof node.text==="function")node.text(arg)})},tpl:function(arg){return array.each(this,function(i){utility.tpl(arg,i)})},toggleClass:function(arg){return array.each(this,function(i){element.toggleClass(i,arg)})},total:function(){return array.total(this)},
toObject:function(){return array.toObject(this)},un:function(event,id,state){return array.each(this,function(i){observer.remove(i,event,id,state)})},unique:function(){return array.unique(this)},update:function(arg){return array.each(this,function(i){element.update(i,arg)})},val:function(arg){var a=[],type=null,same=true;array.each(this,function(i){if(type!==null)same=type===i.type;type=i.type;if(typeof i.val==="function")a.push(element.val(i,arg))});return same?a[0]:a},validate:function(){var result=
[];array.each(this,function(i){result.push(element.validate(i))});return result},zip:function(){return array.zip(this,arguments)}},element:{addClass:function(arg){return element.klass(this,arg,true)},after:function(type,args){return element.create(type,args,this,"after")},append:function(type,args){return element.create(type,args,this,"last")},attr:function(key,value){return element.attr(this,key,value)},before:function(type,args){return element.create(type,args,this,"before")},clear:function(){return element.clear(this)},
create:function(type,args,position){return element.create(type,args,this,position)},css:function(key,value){return element.css(this,key,value)},data:function(key,value){return element.data(this,key,value)},destroy:function(){return element.destroy(this)},disable:function(){return element.disable(this)},dispatch:function(event,data,bubbles,cancelable){return element.dispatch(this,event,data,bubbles,cancelable)},enable:function(){return element.enable(this)},find:function(arg){return element.find(this,
arg)},fire:function(){return observer.fire.apply(observer,[this].concat(array.cast(arguments)))},genId:function(){return utility.genId(this)},get:function(uri,success,failure,headers,timeout){var self=this,defer=deferred();defer.then(function(arg){element.html(self,arg);observer.fire(self,"afterGet");if(typeof success==="function")success.call(self,arg)},function(e){element.html(self,e||label.error.serverError);observer.fire(self,"failedGet");if(typeof failure==="function")failure.call(self,e);throw e;
});observer.fire(this,"beforeGet");uri.get(function(arg){defer.resolve(arg)},function(e){defer.reject(e)},headers,timeout);return defer},has:function(arg){return element.has(this,arg)},hasClass:function(arg){return element.hasClass(this,arg)},html:function(arg){return element.html(this,arg)},is:function(arg){return element.is(this,arg)},isAlphaNum:function(){return element.isAlphaNum(this)},isBoolean:function(){return element.isBoolean(this)},isChecked:function(){return element.isChecked(this)},isDate:function(){return element.isDate(this)},
isDisabled:function(){return element.isDisabled(this)},isDomain:function(){return element.isDomain(this)},isEmail:function(){return element.isEmail(this)},isEmpty:function(){return element.isEmpty(this)},isHidden:function(){return element.hidden(this)},isIP:function(){return element.isIP(this)},isInt:function(){return element.isInt(this)},isNumber:function(){return element.isNumber(this)},isPhone:function(){return element.isPhone(this)},isUrl:function(){return element.isUrl(this)},jsonp:function(uri,
property,callback){var target=this,arg=property;return client.jsonp(uri,function(response){var self=target,node=response,prop=arg,result;try{if(prop!==undefined){prop=prop.replace(/\]|'|"/g,"").replace(/\./g,"[").split("[");prop.each(function(i){node=node[!!isNaN(i)?i:number.parse(i,10)];if(node===undefined)throw new Error(label.error.propertyNotFound);});result=node}else result=response}catch(e){result=label.error.serverError;utility.error(e,arguments,this)}element.html(self,result)},function(e){element.html(target,
label.error.serverError);throw e;},callback)},listeners:function(event){return observer.list(this,event)},loading:function(){return utility.loading(this)},on:function(event,listener,id,scope,state){return observer.add(this,event,listener,id,scope||this,state)},once:function(event,listener,id,scope,state){return observer.once(this,event,listener,id,scope||this,state)},prepend:function(type,args){return element.create(type,args,this,"first")},prependChild:function(child){return element.prependChild(this,
