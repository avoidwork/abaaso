...
typeof data!="object"))throw new Error(label.error.invalidArguments);sync=sync===true;var self=this,events=this.events,defer=deferred(),deferreds=[];if(events)observer.fire(self.parentNode,"beforeDataBatch",data);if(sync)this.clear(sync);if(data.length===0)defer.resolve(this.records);else{if(type==="del")array.each(data,function(i){deferreds.push(self.del(i,false,true))});else array.each(data,function(i){deferreds.push(self.set(null,i,true))});utility.when(deferreds).then(function(){self.loaded=true;
if(events)observer.fire(self.parentNode,"afterDataBatch",self.records);array.each(self.datalists,function(i){i.refresh(true)});if(type==="del")self.reindex();if(self.autosave)self.save();defer.resolve(self.records)},function(e){observer.fire(self.parentNode,"failedDataBatch",e);defer.reject(e)})}return defer};DataStore.prototype.buildUri=function(key){var parsed=utility.parse(this.uri);return parsed.protocol+"//"+parsed.host+parsed.pathname+(regex.endslash.test(parsed.pathname)?"":"/")+key};DataStore.prototype.clear=
function(sync){sync=sync===true;var events=this.events===true;if(!sync){if(events)observer.fire(this.parentNode,"beforeDataClear");array.each(this.datalists,function(i){i.teardown(true)});this.autosave=false;this.callback=null;this.collections=[];this.credentials=null;this.datalists=[];this.depth=0;this.events=true;this.expires=null;this.headers={Accept:"application/json"};this.ignore=[];this.key=null;this.keys={};this.leafs=[];this.loaded=false;this.maxDepth=0;this.pointer=null;this.records=[];this.retrieve=
false;this.source=null;this.total=0;this.views={};this.uri=null;if(events)observer.fire(this.parentNode,"afterDataClear")}else{this.collections=[];this.keys={};this.loaded=false;this.records=[];this.total=0;this.views={};array.each(this.datalists,function(i){i.refresh(true,true)})}return this};DataStore.prototype.crawl=function(arg){var self=this,events=this.events===true,record=arg instanceof Object?arg:this.get(arg),defer=deferred(),deferreds=[],parsed=utility.parse(this.uri||"");if(this.uri===
null||record===undefined)throw new Error(label.error.invalidArguments);if(events)observer.fire(this.parentNode,"beforeDataRetrieve",record);utility.iterate(record.data,function(v,k){var uri;if(array.contains(self.ignore,k)||(array.contains(self.leafs,k)||(self.depth>=self.maxDepth||(!(v instanceof Array)&&typeof v!="string"||v.indexOf("//")===-1&&v.charAt(0)!=="/"))))return;array.add(self.collections,k);record.data[k]=datastore.decorator({id:record.key+"-"+k},null,{key:self.key,pointer:self.pointer,
...
function(v,k){var queries=string.explode(val);queries=queries.filter(function(i){return!string.isEmpty(i)});array.each(queries,function(i,idx){this[idx]="^"+string.escape(i).replace("\\*",".*")});this[k]=queries.join(",")});self.datalist.filter=self.filters}else self.datalist.filter=null;self.datalist.pageIndex=1;self.datalist.refresh(true,self.datalist.store.datalists.length>1)},this.debounce,this.element.id+"Debounce");return this};var grid=function(obj,store,fields,sortable,options,filtered,debounce){var ref=
...
this.sortOrder=sortOrder||(sortable||[])}DataGrid.prototype.constructor=DataGrid;DataGrid.prototype.dump=function(){return this.store.dump(this.list.records,this.fields)};DataGrid.prototype.init=function(debounce){var self,ref,template,container,header,width,css,sort;if(!this.initialized){self=this;ref=[];template="";container=element.create("section",{"class":"grid"},this.element);header=element.create("ul",{"class":"header"},container);width=100/this.fields.length+"%";css="display:inline-block;width:"+
width;sort=this.options.order?string.explode(this.options.order):[];array.each(this.fields,function(i){var trimmed=i.replace(/.*\./g,""),obj=header.create("span",{innerHTML:string.capitalize(string.unCamelCase(string.unhyphenate(trimmed,true)),true),style:css,"class":trimmed,"data-field":i});if(self.sortable.contains(i)){element.klass(obj,"sortable",true);if(sort.filter(function(x){return x.indexOf(i)===0}).length>0)element.data(obj,"sort",array.contains(sort,i+" desc")?"desc":"asc")}template+='<span class="'+
i+'" data-field="'+i+'" style="'+css+'">{{'+i+"}}</span>"});if(this.sortable.length>0)observer.add(header,"click",this.sort,"sort",this);ref.push(datalist.factory(container,this.store,template,this.options));this.list=ref[0];if(this.filtered===true){ref.push(filter(element.create("input",{"class":"filter"},container,"first"),ref[0],this.fields.join(","),debounce||250));this.filter=ref[1]}this.initialized=true}return this};DataGrid.prototype.refresh=function(){var sort=[],self=this;if(this.sortOrder.length>
0){array.each(this.sortOrder,function(i){var obj=element.find(self.element,".header span[data-field='"+i+"']")[0];sort.push(string.trim(i+" "+(element.data(obj,"sort")||"")))});this.options.order=this.list.order=sort.join(", ")}this.list.where=null;utility.merge(this.list,this.options);this.list.refresh();return this};DataGrid.prototype.sort=function(e){var target=utility.target(e),field;utility.stop(e);if(element.hasClass(target,"sortable")){field=element.data(target,"field");element.data(target,
"sort",element.data(target,"sort")==="asc"?"desc":"asc");array.remove(this.sortOrder,field);this.sortOrder.splice(0,0,field);this.refresh()}return this};DataGrid.prototype.teardown=function(){if(this.filter!==null)this.filter.teardown();this.list.teardown();observer.remove(element.find(this.element,".header")[0],"click","sort");element.destroy(element.find(this.element,".grid")[0]);return this};var json={csv:function(arg,delimiter,header){delimiter=delimiter||",";header=header!==false;var obj=json.decode(arg,
true)||arg,result="",prepare;prepare=function(input){var output;if(input instanceof Array){output='"'+input.toString()+'"';if(regex.object_type.test(output))output='"'+json.csv(input,delimiter)+'"'}else if(input instanceof Object)output='"'+json.csv(input,delimiter)+'"';else if(regex.csv_quote.test(input))output='"'+input.replace(/"/g,'""')+'"';else output=input;return output};if(obj instanceof Array)if(obj[0]instanceof Object){if(header)result=array.keys(obj[0]).join(delimiter)+"\n";result+=obj.map(function(i){return json.csv(i,
delimiter,false)}).join("\n")}else result+=prepare(obj,delimiter)+"\n";else{if(header)result=array.keys(obj).join(delimiter)+"\n";result+=array.cast(obj).map(prepare).join(delimiter)+"\n"}return result.replace(/\n$/,"")},decode:function(arg,silent){try{return JSON.parse(arg)}catch(e){if(silent!==true)utility.error(e,arguments,this);return undefined}},encode:function(arg,silent){try{return JSON.stringify(arg)}catch(e){if(silent!==true)utility.error(e,arguments,this);return undefined}}};var label={common:{back:"Back",
...
undefined)item=new LRUItem(value);else item.value=value;item.next=null;item.previous=this.first;this.cache[key]=item;if(this.first!==null)this.cache[this.first].next=key;this.first=key;if(this.last===null)this.last=key;if(++this.length>this.max)this.evict();return this};function LRUItem(value){this.next=null;this.previous=null;this.value=value}LRUItem.prototype.constructor=LRUItem;var math={bezier:function(){var a=array.cast(arguments),t=a.pop(),P=array.chunk(a,2),n=P.length,c,S0,Q0,Q1,Q2,C0,C1,C2,
C3;if(n<2||n>4)throw new Error(label.error.invalidArguments);c=[];S0=1-t;Q0=math.sqr(S0);Q1=2*S0*t;Q2=math.sqr(t);C0=Math.pow(S0,3);C1=3*Q0*t;C2=3*S0*Q2;C3=Math.pow(t,3);if(n===2){c.push(S0*P[0][0]+t*P[1][0]);c.push(S0*P[0][1]+t*P[1][1])}else if(n===3){c.push(Q0*P[0][0]+Q1*P[1][0]+(Q2+P[2][0]));c.push(Q0*P[0][1]+Q1*P[1][1]+(Q2+P[2][1]))}else if(n===4){c.push(C0*P[0][0]+C1*P[1][0]+C2*P[2][0]+C3*P[3][0]);c.push(C0*P[0][1]+C1*P[1][1]+C2*P[2][1]+C3*P[3][1])}return c},dist:function(a,b){return Math.sqrt(math.sqr(b[0]-
...
typeof arg;if(type==="object"){var v=document.body,x=arg.pageX?arg.pageX:v.scrollLeft+arg.clientX,y=arg.pageY?arg.pageY:v.scrollTop+arg.clientY,c=false;if(mouse.pos.x!==x)c=true;abaaso.mouse.prev.x=mouse.prev.x=number.parse(mouse.pos.x,10);abaaso.mouse.pos.x=mouse.pos.x=x;abaaso.mouse.diff.x=mouse.diff.x=mouse.pos.x-mouse.prev.x;if(mouse.pos.y!==y)c=true;abaaso.mouse.prev.y=mouse.prev.y=number.parse(mouse.pos.y,10);abaaso.mouse.pos.y=mouse.pos.y=y;abaaso.mouse.diff.y=mouse.diff.y=mouse.pos.y-mouse.prev.y;
if(c&&abaaso.mouse.log)utility.log([mouse.pos.x,mouse.pos.y,mouse.diff.x,mouse.diff.y])}else if(type==="boolean"){arg?observer.add(document,"mousemove",mouse.track,"tracking"):observer.remove(document,"mousemove","tracking");abaaso.mouse.enabled=mouse.enabled=arg}return abaaso.mouse}};var number={diff:function(num1,num2){if(isNaN(num1)||isNaN(num2))throw new Error(label.error.expectedNumber);return Math.abs(num1-num2)},even:function(arg){return arg%2===0},format:function(arg,delimiter,every){if(isNaN(arg))throw new Error(label.error.expectedNumber);
arg=arg.toString();delimiter=delimiter||",";every=every||3;var d=arg.indexOf(".")>-1?"."+arg.replace(regex.number_format_1,""):"",a=arg.replace(regex.number_format_2,"").split("").reverse(),p=Math.floor(a.length/every),i=1,n,b;for(b=0;b<p;b++){n=i===1?every:every*i+(i===2?1:i-1);a.splice(n,0,delimiter);i++}a=a.reverse().join("");if(a.charAt(0)===delimiter)a=a.substring(1);return a+d},half:function(a,b){return b!==undefined?a/b===0.5:a/2},odd:function(arg){return!number.even(arg)},parse:function(arg,
base){return base===undefined?parseFloat(arg):parseInt(arg,base)},random:function(arg){arg=arg||100;return Math.floor(Math.random()*(arg+1))},round:function(arg,direction){arg=number.parse(arg);if(direction===undefined||string.isEmpty(direction))return number.parse(arg.toFixed(0));else if(regex.down.test(direction))return~~arg;else return Math.ceil(arg)}};var observer={elisteners:{},ignore:false,listeners:{},log:false,maxListeners:10,queue:[],silent:false,add:function(obj,events,fn,id,scope,st){var oId=
observer.id(obj),instance=regex.observer_globals.test(oId)||!/\//g.test(oId)&&oId!=="abaaso"?obj:null,jsonp=/^afterjsonp$/i,add=false,reg=false;if(!oId||(!events||typeof fn!="function"))throw new Error(label.error.invalidArguments);id=id||utility.uuid();scope=scope||obj;st=st||state.getCurrent();if(instance){add=typeof instance.addEventListener=="function";reg=typeof instance.attachEvent=="object"||add}if(!observer.listeners[oId]){observer.listeners[oId]={};observer.listeners[oId].all={}}if(st!==
"all"&&!observer.listeners[oId][st])observer.listeners[oId][st]={};array.each(string.explode(events),function(ev){var eId=oId+"_"+ev;if(!observer.listeners[oId].all[ev])observer.listeners[oId].all[ev]=lru(observer.maxListeners);if(st!=="all"&&!observer.listeners[oId][st][ev])observer.listeners[oId][st][ev]=lru(observer.maxListeners);if(reg&&(!jsonp.test(ev)&&!observer.elisteners[eId])){observer.elisteners[eId]=function(e){observer.fire(oId,ev,e)};instance[add?"addEventListener":"attachEvent"]((add?
"":"on")+ev,observer.elisteners[eId],false)}observer.listeners[oId][st][ev].set(id,{fn:fn,scope:scope})});return obj},decorate:function(obj){var methods=[["fire",function(){return observer.fire.apply(observer,[this].concat(array.cast(arguments)))}],["listeners",function(e){return observer.list(this,e)}],["on",function(e,listener,id,scope,standby){return observer.add(this,e,listener,id,scope,standby)}],["once",function(e,listener,id,scope,standby){return observer.once(this,e,listener,id,scope,standby)}],
["un",function(e,id){return observer.remove(this,e,id)}]];array.each(methods,function(i){utility.property(obj,i[0],{value:i[1],configurable:true,enumerable:true,writable:true})});return obj},discard:function(arg){return arg===undefined?observer.ignore:observer.ignore=arg===true},fire:function(obj,events){var args,log,cache,item,oId;if(!observer.ignore)if(observer.silent)observer.queue.push(array.cast(arguments));else{oId=observer.id(obj);if(observer.listeners[oId]){args=array.remove(array.cast(arguments),
0,1);log=abaaso.logging;array.each(string.explode(events),function(ev){if(log)utility.log(oId+" fired "+ev);array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&observer.listeners[oId][st][ev]){cache=observer.listeners[oId][st][ev];if(cache.length>0){item=cache.cache[cache.last];do if(item.value.fn.apply(item.value.scope,args)!==false&&item.next)item=cache.cache[item.next];else return false;while(item)}}})})}}return obj},id:function(arg){var id;if(arg===global)id="window";else if(!server&&
...
string.explode(events);st=st||state.getCurrent();states=array.keys(observer.listeners[oId]);total=0;array.each(states,function(s){array.each(events,function(e){if(observer.listeners[oId][s][e])total+=observer.listeners[oId][s][e].length})});array.each(events,function(ev){if(observer.listeners[oId][st][ev])if(id){observer.listeners[oId][st][ev].remove(id);if(--total===0){array.each(states,function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}}else{array.each(states,
function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}})}return obj},states:function(){return["all",state.getCurrent()]}};var promise={delay:function(){if(typeof setImmediate!="undefined")return setImmediate;else if(typeof process!="undefined")return process.nextTick;else return function(arg){setTimeout(arg,0)}}(),factory:function(){return new Promise},pipe:function(parent,child){parent.then(function(arg){child.resolve(arg)},function(e){child.reject(e)})},
state:{PENDING:0,FAILURE:1,SUCCESS:2}};function Promise(){this.deferred=false;this.handlers=[];this.state=promise.state.PENDING;this.value=null}Promise.prototype.constructor=Promise;Promise.prototype.process=function(){var result,success,value;this.deferred=false;if(this.state===promise.state.PENDING)return;value=this.value;success=this.state===promise.state.SUCCESS;array.each(this.handlers.slice(),function(i){var callback=i[success?"success":"failure"],child=i.promise;if(!callback||typeof callback!=
"function"){if(value&&typeof value.then=="function")promise.pipe(value,child);else if(success)child.resolve(value);else child.reject(value);return}try{result=callback(value)}catch(e){child.reject(e);return}if(result&&typeof result.then=="function")promise.pipe(result,promise);else child.resolve(result)});return this};Promise.prototype.reject=function(arg){var self=this;if(this.state>promise.state.PENDING)return;this.value=arg;this.state=promise.state.FAILURE;if(!this.deferred){promise.delay(function(){self.process()});
this.deferred=true}return this};Promise.prototype.resolve=function(arg){var self=this;if(this.state>promise.state.PENDING)return;this.value=arg;this.state=promise.state.SUCCESS;if(!this.deferred){promise.delay(function(){self.process()});this.deferred=true}return this};Promise.prototype.then=function(success,failure){var self=this,child=new Promise;this.handlers.push({success:success,failure:failure,promise:child});if(this.state>promise.state.PENDING&&!this.deferred){promise.delay(function(){self.process()});
this.deferred=true}return child};var prototypes={array:{add:function(arg){return array.add(this,arg)},addClass:function(arg){return array.each(this,function(i){element.klass(i,arg)})},after:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,"after"))});return result},append:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,"last"))});return result},attr:function(key,value){var result=[];array.each(this,
function(i){result.push(element.attr(i,key,value))});return result},before:function(type,args){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,"before"))});return result},binIndex:function(arg){return array.binIndex(this,arg)},chunk:function(size){return array.chunk(this,size)},clear:function(){return!server&&this[0]instanceof Element?array.each(this,function(i){element.clear(i)}):array.clear(this)},clone:function(){return utility.clone(this)},collect:function(arg){return array.collect(this,
arg)},compact:function(){return array.compact(this)},contains:function(arg){return array.contains(this,arg)},count:function(arg){return array.count(this,arg)},create:function(type,args,position){var result=[];array.each(this,function(i){result.push(element.create(type,args,i,position))});return result},css:function(key,value){return array.each(this,function(i){element.css(i,key,value)})},data:function(key,value){var result=[];array.each(this,function(i){result.push(element.data(i,key,value))});return result},
diff:function(arg){return array.diff(this,arg)},disable:function(){return array.each(this,function(i){element.disable(i)})},dispatch:function(event,data,bubbles,cancelable){return array.each(this,function(i){element.dispatch(i,event,data,bubbles,cancelable)})},destroy:function(){array.each(this,function(i){element.destroy(i)});return[]},each:function(arg,async,size){return array.each(this,arg,async,size)},empty:function(){return array.empty(this)},enable:function(){return array.each(this,function(i){element.enable(i)})},
equal:function(arg){return array.equal(this,arg)},fib:function(arg){return array.fib(arg)},fill:function(arg,start,offset){return array.fill(this,arg,start,offset)},find:function(arg){var result=[];array.each(this,function(i){element.find(i,arg).each(function(r){result.add(r)})});return result},fire:function(){var args=arguments;return array.each(this,function(i){observer.fire.apply(observer,[i].concat(array.cast(args)))})},first:function(){return array.first(this)},flat:function(){return array.flat(this)},
genId:function(){return array.each(this,function(i){utility.genId(i)})},get:function(uri,headers){var result=[];array.each(this,function(i,idx){i.get(uri,headers,function(arg){result[idx]=arg},function(e){result[idx]=e})});return result},has:function(arg){var result=[];array.each(this,function(i){result.push(element.has(i,arg))});return result},hasClass:function(arg){var result=[];array.each(this,function(i){result.push(element.hasClass(i,arg))});return result},html:function(arg){var result;if(arg!==
undefined)return array.each(this,function(i){element.html(i,arg)});else{result=[];array.each(this,function(i){result.push(element.html(i))});return result}},index:function(arg){return array.index(this,arg)},indexed:function(){return array.indexed(this)},intersect:function(arg){return array.intersect(this,arg)},is:function(arg){var result=[];array.each(this,function(i){result.push(element.is(i,arg))});return result},isAlphaNum:function(){var result=[];array.each(this,function(i){result.push(i.isAlphaNum())});
return result},isBoolean:function(){var result=[];array.each(this,function(i){result.push(i.isBoolean())});return result},isChecked:function(){var result=[];array.each(this,function(i){result.push(i.isChecked())});return result},isDate:function(){var result=[];array.each(this,function(i){result.push(i.isDate())});return result},isDisabled:function(){var result=[];array.each(this,function(i){result.push(element.isDisabled(i))});return result},isDomain:function(){var result=[];array.each(this,function(i){result.push(i.isDomain())});
return result},isEmail:function(){var result=[];array.each(this,function(i){result.push(i.isEmail())});return result},isEmpty:function(){var result=[];array.each(this,function(i){result.push(i.isEmpty())});return result},isHidden:function(){var result=[];array.each(this,function(i){result.push(element.isHidden(i))});return result},isIP:function(){var result=[];array.each(this,function(i){result.push(i.isIP())});return result},isInt:function(){var result=[];array.each(this,function(i){result.push(i.isInt())});
return result},isNumber:function(){var result=[];array.each(this,function(i){result.push(i.isNumber())});return result},isPhone:function(){var result=[];array.each(this,function(i){result.push(i.isPhone())});return result},isUrl:function(){var result=[];array.each(this,function(i){result.push(i.isUrl())});return result},keepIf:function(fn){return array.keepIf(this,fn)},keySort:function(query,sub){return array.keySort(this,query,sub)},keys:function(){return array.keys(this)},last:function(arg){return array.last(this,
...
"PUT",success,failure,args,headers)},on:function(event,listener,id,scope,state){return observer.add(this,event,listener,id,scope,state)},once:function(event,listener,id,scope,state){return observer.once(this,event,listener,id,scope,state)},options:function(success,failure){return client.request(this,"OPTIONS",success,failure)},permissions:function(){return client.permissions(this)},singular:function(){return string.singular(this)},toCamelCase:function(){return string.toCamelCase(this)},toNumber:function(base){return number.parse(this,
...
parsed.hash;parsed.path=obj.path||parsed.pathname+parsed.search;parsed.query=utility.queryString(null,parsed.search);return parsed},prevent:function(e){if(typeof e.preventDefault=="function")e.preventDefault();return e},property:function(obj,prop,descriptor){if(!(descriptor instanceof Object))throw new Error(label.error.invalidArguments);if(descriptor.value!==undefined&&descriptor.get!==undefined)delete descriptor.value;Object.defineProperty(obj,prop,descriptor);return obj},proto:function(obj,type){var target=
obj.prototype||obj;utility.iterate(prototypes[type],function(v,k){if(!target[k])utility.property(target,k,{value:v,configurable:true,writable:true})});return obj},queryString:function(arg,qstring){var obj={},result=qstring!==undefined?qstring.indexOf("?")>-1?qstring.replace(/.*\?/,""):null:server||string.isEmpty(location.search)?null:location.search.replace("?",""),item;if(result!==null&&!string.isEmpty(result)){result=result.split("&");array.each(result,function(prop){item=prop.split("=");if(string.isEmpty(item[0]))return;
if(item[1]===undefined||string.isEmpty(item[1]))item[1]="";else if(string.isNumber(item[1]))item[1]=Number(item[1]);else if(string.isBoolean(item[1]))item[1]=item[1]==="true";if(obj[item[0]]===undefined)obj[item[0]]=item[1];else if(!(obj[item[0]]instanceof Array)){obj[item[0]]=[obj[item[0]]];obj[item[0]].push(item[1])}else obj[item[0]].push(item[1])})}if(arg!==null&&arg!==undefined)obj=obj[arg];return obj},reflect:function(arg){if(arg===undefined)arg=this||utility.$;arg=arg.toString().match(regex.reflect)[1];
return string.explode(arg)},repeat:function(fn,ms,id,now){ms=ms||10;id=id||utility.uuid(true);now=now!==false;if(now&&fn()===false)return;utility.defer(function(){var recursive=function(fn,ms,id){var recursive=this;if(fn()!==false)utility.repeating[id]=setTimeout(function(){recursive.call(recursive,fn,ms,id)},ms);else delete utility.repeating[id]};recursive.call(recursive,fn,ms,id)},ms,id,true);return id},stop:function(e){if(typeof e.stopPropagation=="function")e.stopPropagation();utility.prevent(e);
e.returnValue=false;return e},sugar:function(){utility.proto(Array,"array");if(typeof Element!="undefined")utility.proto(Element,"element");utility.proto(Function,"function");utility.proto(Math,"math");utility.proto(Number,"number");utility.proto(String,"string")},target:function(e){var obj=new Abaaso;obj.push(e.target||e.srcElement);return obj},tpl:function(arg,target){var frag;if(typeof arg!="object"||!regex.object_undefined.test(typeof target)&&(target=target.charAt(0)==="#"?utility.dom(target):
utility.dom(target)[0])===undefined)throw new Error(label.error.invalidArguments);if(target===undefined)target=utility.dom("body")[0];frag=document.createDocumentFragment();if(arg instanceof Array)array.each(arg,function(i){element.html(element.create(array.cast(i,true)[0],frag),array.cast(i)[0])});else utility.iterate(arg,function(v,k){if(typeof v=="string")element.html(element.create(k,undefined,frag),v);else if(v instanceof Array||v instanceof Object)utility.tpl(v,element.create(k,undefined,frag))});
target.appendChild(frag);return array.last(target.childNodes)},uuid:function(safe){var s=function(){return((1+Math.random())*65536|0).toString(16).substring(1)},r=[8,9,"a","b"],o;o=s()+s()+"-"+s()+"-4"+s().substr(0,3)+"-"+r[Math.floor(Math.random()*4)]+s().substr(0,3)+"-"+s()+s()+s();if(safe===true)o=o.replace(/-/g,"");return o},walk:function(obj,arg){array.each(arg.replace(/\]$/,"").replace(/\]/g,".").replace(/\.\./g,".").split(/\.|\[/),function(i){obj=obj[i]});return obj},when:function(){var i=
0,defer=deferred(),args=array.cast(arguments),nth;if(args[0]instanceof Array)args=args[0];nth=args.length;if(nth===0)defer.resolve(null);else array.each(args,function(p){p.then(function(){if(++i===nth&&!defer.isResolved())if(args.length>1)defer.resolve(args.map(function(obj){return obj.value||obj.promise.value}));else defer.resolve(args[0].value||args[0].promise.value)},function(){if(!defer.isResolved())if(args.length>1)defer.reject(args.map(function(obj){return obj.value||obj.promise.value}));else defer.reject(args[0].value||
args[0].promise.value)})});return defer}};var validate={test:function(args){var exception=false,invalid=[],value=null,c=[],p;if(args.nodeName!==undefined&&args.nodeName==="FORM"){if(string.isEmpty(args.id))utility.genId(args);c=utility.dom("#"+args.id+" input, #"+args.id+" select");array.each(c,function(i){var z={},p,v,r;p=regex[i.nodeName.toLowerCase()]?regex[i.nodeName.toLowerCase()]:!string.isEmpty(i.id)&&regex[i.id.toLowerCase()]?regex[i.id.toLowerCase()]:"notEmpty";v=element.val(i);if(v===null)v=
"";z[p]=v;r=validate.test(z);if(!r.pass){invalid.push({element:i,test:p,value:v});exception=true}})}else utility.iterate(args,function(v,k){if(v===undefined||v===null){invalid.push({test:k,value:v});exception=true;return}value=v.toString().charAt(0)==="#"?utility.dom(v)!==undefined?element.val(utility.dom(v)):"":v;if(k==="date"){if(isNaN((new Date(value)).getYear())){invalid.push({test:k,value:value});exception=true}}else if(k==="domain"){if(!regex.domain.test(value.replace(regex.scheme,""))){invalid.push({test:k,
value:value});exception=true}}else if(k==="domainip"){if(!regex.domain.test(value.replace(regex.scheme,""))||!regex.ip.test(value)){invalid.push({test:k,value:value});exception=true}}else{p=regex[k]||k;if(!p.test(value)){invalid.push({test:k,value:value});exception=true}}});return{pass:!exception,invalid:invalid}}};var xhr=function(){var UNSENT=0,OPENED=1,HEADERS_RECEIVED=2,LOADING=3,DONE=4,ready=new RegExp(HEADERS_RECEIVED+"|"+LOADING),XMLHttpRequest,headers,handler,handlerError,state;headers={"User-Agent":"abaaso/4.0.0 node.js/"+
...
undefined&&this._resheaders["set-cookie"]instanceof Array)this._resheaders["set-cookie"]=this._resheaders["set-cookie"].join(";");res.on("data",function(arg){res.setEncoding("utf8");if(self._send){if(arg)self.responseText+=arg;state.call(self,LOADING)}});res.on("end",function(){if(self._send){state.call(self,DONE);self._send=false}})};handlerError=function(e){this.status=503;this.statusText=e;this.responseText=e!==undefined?e.stack||e:e;this._error=true;this._send=false;this.setRequestHeader("Content-Type",
"text/plain");this.dispatchEvent("error");state.call(this,DONE)};XMLHttpRequest=function(){this.onabort=null;this.onerror=null;this.onload=null;this.onloadend=null;this.onloadstart=null;this.onreadystatechange=null;this.readyState=UNSENT;this.response=null;this.responseText="";this.responseType="";this.responseXML=null;this.status=UNSENT;this.statusText="";this._id=utility.genId();this._error=false;this._headers={};this._listeners={};this._params={};this._request=null;this._resheaders={};this._send=
