...
DataStore.prototype.purge=function(arg){return this.storage(arg||this,"remove")};DataStore.prototype.reindex=function(){var nth=this.total,i=-1;this.views={};if(nth>0)while(++i<nth){this.records[i].index=i;this.keys[this.records[i].key]=i}return this};DataStore.prototype.restore=function(arg){return this.storage(arg||this,"get")};DataStore.prototype.save=function(arg){return this.storage(arg||this,"set")};DataStore.prototype.select=function(where){var defer=deferred(),blob,functions,worker;if(!(where instanceof
Object))throw new Error(label.error.invalidArguments);if(server)defer.resolve(this.records.slice().filter(function(rec){var match=true;utility.iterate(where,function(v,k){var type=typeof v;if(type!=="function"&&rec.data[k]!==v)return match=false;else if(type==="function"&&!v(rec.data[k],rec))return match=false});return match}));else{functions=[];utility.iterate(where,function(v,k){if(typeof v==="function"){this[k]=v.toString();functions.push(k)}});blob=new Blob([WORKER]);worker=new Worker(global.URL.createObjectURL(blob));
worker.onmessage=function(ev){defer.resolve(ev.data)};worker.postMessage({cmd:"select",records:this.records.slice(),where:json.encode(where),functions:functions})}return defer};DataStore.prototype.set=function(key,data,batch){data=utility.clone(data,true);batch=batch===true;var self=this,events=this.events,defer=deferred(),record=key!==null?this.get(key)||null:null,method="POST",parsed=utility.parse(self.uri||""),uri;if(typeof data==="string"){if(data.indexOf("//")===-1)if(data.charAt(0)!=="/")uri=
this.buildUri(data);else if(self.uri!==null&&regex.root.test(data))uri=parsed.protocol+"//"+parsed.host+data;else uri=data;else uri=data;key=uri.replace(regex.not_endpoint,"");if(string.isEmpty(key))defer.reject(new Error(label.error.invalidArguments));else{if(!batch&&events)observer.fire(self.parentNode,"beforeDataSet",{key:key,data:data});client.request(uri,"GET",function(arg){self.setComplete(record,key,self.source?arg[self.source]:arg,batch,defer)},function(e){observer.fire(self.parentNode,"failedDataSet",
e);defer.reject(e)},undefined,utility.merge({withCredentials:self.credentials},self.headers))}}else{if(record===null&&(key===null||key===undefined))if(this.key===null)key=utility.uuid();else if(data[this.key]){key=data[this.key];delete data[this.key]}else key=utility.uuid();if(!batch&&events)observer.fire(self.parentNode,"beforeDataSet",{key:key,data:data});if(batch||this.uri===null)this.setComplete(record,key,data,batch,defer);else{if(key!==null){method="PUT";uri=this.buildUri(key);if(client.allows(uri,
"patch")&&(!client.ie||client.activex))method="PATCH";else if(record!==null)utility.iterate(record.data,function(v,k){if(!array.contains(self.collections,k)&&!data[k])data[k]=v})}else uri=this.uri;client.request(uri,method,function(arg){self.setComplete(record,key,self.source?arg[self.source]:arg,batch,defer)},function(e){observer.fire(self.parentNode,"failedDataSet",e);defer.reject(e)},data,utility.merge({withCredentials:this.credentials},this.headers))}}return defer};DataStore.prototype.setComplete=
...
if(!batch&&this.events)observer.fire(self.parentNode,"afterDataSet",record);if(deferreds.length===0)defer.resolve(record);else utility.when(deferreds).then(function(){defer.resolve(record)});return this};DataStore.prototype.setExpires=function(arg){if(arg!==null&&this.uri===null||arg!==null&&(isNaN(arg)||arg<1E3))throw new Error(label.error.invalidArguments);if(this.expires===arg)return;this.expires=arg;var id=this.parentNode.id+"DataExpire",expires=arg,self=this;utility.clearTimers(id);if(arg===
null)return;utility.repeat(function(){if(self.uri===null){self.setExpires(null);return false}if(!cache.expire(self.uri))observer.fire(self.uri,"beforeExpire, expire, afterExpire")},expires,id,false)};DataStore.prototype.setUri=function(arg){var defer=deferred();if(arg!==null&&string.isEmpty(arg))throw new Error(label.error.invalidArguments);arg=utility.parse(arg).href;if(this.uri===arg)defer.resolve(this.records);else{if(this.uri!==null)observer.remove(this.uri);this.uri=arg;if(this.uri!==null){observer.add(this.uri,
"expire",function(){this.sync()},"dataSync",this);cache.expire(this.uri,true);this.sync().then(function(arg){defer.resolve(arg)},function(e){defer.reject(e)})}}return defer};DataStore.prototype.sort=function(query,create,where){create=create===true||where instanceof Object;var self=this,view=string.explode(query).join(" ").toCamelCase(),defer=deferred(),blob,next,worker;next=function(records){if(self.total===0)defer.resolve([]);else if(!create&&self.views[view])defer.resolve(self.views[view]);else if(server){self.views[view]=
array.keySort(records.slice(),query,"data");defer.resolve(self.views[view])}else{blob=new Blob([WORKER]);worker=new Worker(global.URL.createObjectURL(blob));worker.onmessage=function(ev){self.views[view]=ev.data;defer.resolve(self.views[view])};worker.postMessage({cmd:"sort",records:records.slice(),query:query})}};if(!where)next(this.records);else this.select(where).then(next);return defer};DataStore.prototype.storage=function(obj,op,type){var self=this,record=false,mongo=!string.isEmpty(this.mongodb),
session=type==="session"&&typeof sessionStorage!=="undefined",defer=deferred(),data,deferreds,key,result;if(!regex.number_string_object.test(typeof obj)||!regex.get_remove_set.test(op))throw new Error(label.error.invalidArguments);record=regex.number_string.test(typeof obj)||obj.hasOwnProperty("key")&&!obj.hasOwnProperty("parentNode");if(op!=="remove"){if(record&&!(obj instanceof Object))obj=this.get(obj);key=record?obj.key:obj.parentNode.id}else if(op==="remove"&&record)key=obj.key||obj;if(op===
"get")if(mongo)mongodb.connect(this.mongodb,function(e,db){if(e){defer.reject(e);db.close()}else db.createCollection(self.parentNode.id,function(e,collection){if(e){defer.reject(e);db.close()}else if(record)collection.find({_id:obj.key}).limit(1).toArray(function(e,recs){if(e)defer.reject(e);else{delete recs[0]._id;self.set(key,recs[0],true).then(function(rec){defer.resolve(rec)},function(e){defer.reject(e)})}db.close()});else collection.find({}).toArray(function(e,recs){var i=-1,nth=recs.length;
...
"remove")if(mongo)mongodb.connect(this.mongodb,function(e,db){if(e){defer.reject(e);db.close()}else db.createCollection(self.parentNode.id,function(e,collection){collection.remove(record?{_id:key}:{},{safe:true},function(e,arg){if(e)defer.reject(e);else defer.resolve(arg);db.close()})})});else{session?sessionStorage.removeItem(key):localStorage.removeItem(key);defer.resolve(this)}else if(op==="set")if(mongo)mongodb.connect(this.mongodb,function(e,db){if(e){defer.reject(e);db.close()}else db.createCollection(self.parentNode.id,
function(e,collection){if(e){defer.reject(e);db.close()}else if(record)collection.update({_id:obj.key},{$set:obj.data},{w:1,safe:true,upsert:true},function(e,arg){if(e)defer.reject(e);else defer.resolve(arg);db.close()});else collection.remove({},{w:1,safe:true},function(e){if(e){defer.reject(e);db.close()}else{deferreds=[];array.each(self.records,function(i){var data={},defer2=deferred();deferreds.push(defer2);utility.iterate(i.data,function(v,k){if(!array.contains(self.collections,k))data[k]=v});
collection.update({_id:i.key},{$set:data},{w:1,safe:true,upsert:true},function(e,arg){if(e)defer2.reject(e);else defer2.resolve(arg)})});utility.when(deferreds).then(function(result){defer.resolve(result);db.close()},function(e){defer.reject(e);db.close()})}})})});else{data=json.encode(record?obj.data:{total:this.total,keys:this.keys,records:this.records});session?sessionStorage.setItem(key,data):localStorage.setItem(key,data);defer.resolve(this)}return defer};DataStore.prototype.sync=function(){if(this.uri===
null||string.isEmpty(this.uri))throw new Error(label.error.invalidArguments);var self=this,events=this.events===true,defer=deferred(),success,failure;success=function(arg){var data;if(typeof arg!=="object")throw new Error(label.error.expectedObject);if(self.source!==null)arg=utility.walk(arg,self.source);if(arg instanceof Array)data=arg;else data=[arg];self.batch("set",data,true).then(function(arg){if(events)observer.fire(self.parentNode,"afterDataSync",arg);defer.resolve(arg)},failure)};failure=
