...
while(item)}}})})}}return obj},id:function(arg){var id;if(arg===global)id="window";else if(!server&&arg===document)id="document";else if(!server&&arg===document.body)id="body";else{utility.genId(arg);id=arg.id||(typeof arg.toString==="function"?arg.toString():arg)}return id},list:function(obj,ev){var oId=observer.id(obj),result={},events,states;if(!observer.listeners[oId]);else if(!ev){events=[];states=array.keys(observer.listeners[oId]);array.each(states,function(st){array.merge(events,array.keys(observer.listeners[oId][st]))});
array.each(events,function(ev){result[ev]={}});array.each(states,function(st){if(observer.listeners[oId][st])array.each(array.keys(observer.listeners[oId][st]),function(ev){if(observer.listeners[oId][st][ev].length>0){result[ev][st]={};utility.iterate(observer.listeners[oId][st][ev].cache,function(v,k){result[ev][st][k]=v.value})}})})}else array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&(observer.listeners[oId][st][ev]&&observer.listeners[oId][st][ev].length>0)){result[st]=
{};utility.iterate(observer.listeners[oId][st][ev].cache,function(v,k){result[st][k]=v.value})}});return result},once:function(obj,events,fn,id,scope,st){id=id||utility.uuid();scope=scope||obj;st=st||state.getCurrent();if(!obj||(!events||typeof fn!=="function"))throw new Error(label.error.invalidArguments);array.each(string.explode(events),function(ev){observer.add(obj,ev,function(){fn.apply(scope,arguments);observer.remove(obj,ev,id,st)},id,scope,st)});return obj},pause:function(arg){if(arg===true)observer.silent=
...
function(v,k){if(k.indexOf(oId+"_")===0)unhook.call(this,k,k.replace(regex,""))});delete observer.listeners[oId]}else{events=string.explode(events);states=array.keys(observer.listeners[oId]);total=0;array.each(states,function(s){array.each(events,function(e){if(observer.listeners[oId][s][e])total+=observer.listeners[oId][s][e].length})});array.each(events,function(ev){if(observer.listeners[oId][st][ev])if(id){observer.listeners[oId][st][ev].remove(id);if(--total===0){array.each(states,function(s){delete observer.listeners[oId][s][ev]});
unhook.call(observer.elisteners,oId+"_"+ev,ev)}}else{array.each(states,function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}})}return obj},states:function(){return["all",state.getCurrent()]}};var promise={delay:function(){if(typeof setImmediate!=="undefined")return setImmediate;else if(typeof process!=="undefined")return process.nextTick;else return function(arg){setTimeout(arg,0)}}(),factory:function(){return new Promise},pipe:function(parent,child){parent.then(function(arg){child.resolve(arg)},
function(e){child.reject(e)})},state:{PENDING:0,FAILURE:1,SUCCESS:2}};function Promise(){this.deferred=false;this.handlers=[];this.state=promise.state.PENDING;this.value=null}Promise.prototype.constructor=Promise;Promise.prototype.process=function(){var result,success,value;this.deferred=false;if(this.state===promise.state.PENDING)return;value=this.value;success=this.state===promise.state.SUCCESS;array.each(this.handlers.slice(),function(i){var callback=i[success?"success":"failure"],child=i.promise;
if(!callback||typeof callback!=="function"){if(value&&typeof value.then==="function")promise.pipe(value,child);else if(success)child.resolve(value);else child.reject(value);return}try{result=callback(value)}catch(e){child.reject(e);return}if(result&&typeof result.then==="function")promise.pipe(result,promise);else child.resolve(result)});return this};Promise.prototype.reject=function(arg){var self=this;if(this.state>promise.state.PENDING)return;this.value=arg;this.state=promise.state.FAILURE;if(!this.deferred){promise.delay(function(){self.process()});
