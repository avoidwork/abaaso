...
else return Math.ceil(arg)}};var observer={elisteners:{},ignore:false,listeners:{},log:false,maxListeners:10,queue:[],silent:false,add:function(obj,events,fn,id,scope,st){var oId=observer.id(obj),instance=regex.observer_globals.test(oId)||!/\//g.test(oId)&&oId!=="abaaso"?obj:null;id=id||utility.uuid();scope=scope||obj;st=st||state.getCurrent();if(!observer.listeners[oId]){observer.listeners[oId]={};observer.listeners[oId].all={}}if(st!=="all"&&!observer.listeners[oId][st])observer.listeners[oId][st]=
{};array.each(string.explode(events),function(ev){var eId=oId+"_"+ev,add,reg;if(!observer.listeners[oId].all[ev])observer.listeners[oId].all[ev]=lru(observer.maxListeners);if(st!=="all"&&!observer.listeners[oId][st][ev])observer.listeners[oId][st][ev]=lru(observer.maxListeners);if(instance&&(ev.toLowerCase()!=="afterjsonp"&&(!observer.elisteners[eId]&&(regex.observer_globals.test(oId)||obj.nodeName)))){add=typeof instance.addEventListener==="function";reg=typeof instance.attachEvent==="object"||add;
if(reg){observer.elisteners[eId]=function(e){observer.fire(oId,ev,e)};instance[add?"addEventListener":"attachEvent"]((add?"":"on")+ev,observer.elisteners[eId],false)}}observer.listeners[oId][st][ev].set(id,{fn:fn,scope:scope})})},decorate:function(){var methods=[["fire",function(){return observer.fire.apply(observer,[this].concat(array.cast(arguments)))}],["listeners",function(e){return observer.list(this,e)}],["on",function(e,listener,id,scope,standby){return observer.add(this,e,listener,id,scope,
standby)}],["once",function(e,listener,id,scope,standby){return observer.once(this,e,listener,id,scope,standby)}],["un",function(e,id){return observer.remove(this,e,id)}]];array.each(methods,function(i){utility.property(obj,i[0],{value:i[1],configurable:true,enumerable:true,writable:true})});return obj},discard:function(arg){return arg===undefined?observer.ignore:observer.ignore=arg===true},fire:function(obj,events){var args,log,cache,item,oId;if(!observer.ignore)if(observer.silent)observer.queue.push(array.cast(arguments));
...
function(ev){result[st]=observer.listeners[oId][st][ev].cache})});else array.each(observer.states(),function(st){if(observer.listeners[oId][st]&&observer.listeners[oId][st][ev])result[st]=observer.listeners[oId][st][ev].cache});return result},once:function(obj,events,fn,id,scope,st){id=id||utility.uuid();scope=scope||obj;st=st||state.getCurrent();array.each(string.explode(events),function(ev){observer.add(obj,ev,function(){fn.apply(scope,arguments);observer.remove(obj,ev,id,st)},id,scope,st)});return obj},
pause:function(arg){if(arg===true)observer.silent=arg;else if(arg===false){observer.silent=arg;array.each(observer.queue,function(i){observer.fire.apply(observer,i)});observer.queue=[]}return observer.silent},remove:function(obj,events,id,st){var oId=observer.id(obj),add=typeof obj.addEventListener==="function",reg=typeof obj.attachEvent==="object"||add,regex=/.*_/,states,total,unhook;st=st||state.getCurrent();unhook=function(eId,ev){if(reg)obj[add?"removeEventListener":"detachEvent"]((add?"":"on")+
ev,this[eId],false);delete this[eId]};if(observer.listeners[oId])if(!events){utility.iterate(observer.elisteners,function(v,k){if(k.indexOf(oId+"_")===0)unhook.call(this,k,k.replace(regex,""))});delete observer.listeners[oId]}else{events=string.explode(events);states=array.keys(observer.listeners[oId]);total=0;array.each(states,function(s){array.each(events,function(e){if(observer.listeners[oId][s][e])total+=observer.listeners[oId][s][e].length})});array.each(events,function(ev){if(observer.listeners[oId][st][ev])if(id){observer.listeners[oId][st][ev].remove(id);
total--;if(total===0){array.each(states,function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}}else{array.each(states,function(s){delete observer.listeners[oId][s][ev]});unhook.call(observer.elisteners,oId+"_"+ev,ev)}})}return obj},states:function(){return["all",state.getCurrent()]}};var promise={delay:function(){if(typeof setImmediate!=="undefined")return setImmediate;else if(typeof process!=="undefined")return process.nextTick;else return function(arg){setTimeout(arg,
...
timestamp:(new Date).toUTCString(),type:e.type||"TypeError"};utility.log(o.stack||o.message,!warning?"error":"warn");utility.error.log.push(o);observer.fire("abaaso","error",o);return undefined},extend:function(obj,arg){var o;if(obj===undefined)throw new Error(label.error.invalidArguments);o=Object.create(obj);if(arg instanceof Object)utility.merge(o,arg);return o},fib:function(i,r){if(r===true)return i>1?utility.fib(i-1,r)+utility.fib(i-2,r):i;else return array.last(array.fib(i))},genId:function(obj,
dom){dom=dom===true;var id;if(obj!==undefined&&(obj.id!==undefined&&obj.id!==""||(obj instanceof Array||(obj instanceof String||typeof obj==="string"))))return obj;if(dom){do id=utility.domId(utility.uuid(true));while(utility.dom("#"+id)!==undefined)}else id=utility.domId(utility.uuid(true));if(typeof obj==="object"){obj.id=id;return obj}else return id},hash:function(arg){if(arg)document.location.hash=arg;return document.location.hash},hex:function(color){var digits,red,green,blue,result,i,nth;if(color.charAt(0)===
"#")result=color;else{digits=string.explode(color.replace(/.*\(|\)/g,""));red=number.parse(digits[0]||0);green=number.parse(digits[1]||0);blue=number.parse(digits[2]||0);result=(blue|green<<8|red<<16).toString(16);if(result.length<6){nth=number.diff(result.length,6);i=-1;while(++i<nth)result="0"+result}result="#"+result}return result},iterate:function(obj,fn){if(typeof fn!=="function")throw new Error(label.error.invalidArguments);array.each(Object.keys(obj),function(i){return fn.call(obj,obj[i],i)});
return obj},loading:function(obj){var l=abaaso.loading;if(l.url===null||obj===undefined)throw new Error(label.error.invalidArguments);if(l.image===undefined){l.image=new Image;l.image.src=l.url}element.clear(obj);element.create("img",{alt:label.common.loading,src:l.image.src},element.create("div",{"class":"loading"},obj));return obj},log:function(arg,target){var ts,msg;if(typeof console!=="undefined"){ts=typeof arg!=="object";msg=ts?"["+(new Date).toLocaleTimeString()+"] "+arg:arg;console[target||
"log"](msg)}},merge:function(obj,arg){utility.iterate(arg,function(v,k){if(obj[k]instanceof Array&&v instanceof Array)array.merge(obj[k],v);else if(obj[k]instanceof Object&&v instanceof Object)utility.iterate(v,function(x,y){obj[k][y]=utility.clone(x)});else obj[k]=utility.clone(v)});return obj},object:function(obj){return typeof obj==="object"?obj:obj.charAt&&obj.charAt(0)==="#"?utility.dom(obj):obj},parse:function(uri){var obj={},parsed={};if(uri===undefined)uri=!server?location.href:"";if(!server){obj=
...
"element");utility.proto(Function,"function");utility.proto(Math,"math");utility.proto(Number,"number");utility.proto(String,"string")},target:function(e){var obj=new Abaaso;obj.push(e.target||e.srcElement);return obj},tpl:function(arg,target){var frag;if(typeof arg!=="object"||!regex.object_undefined.test(typeof target)&&(target=target.charAt(0)==="#"?utility.dom(target):utility.dom(target)[0])===undefined)throw new Error(label.error.invalidArguments);if(target===undefined)target=utility.dom("body")[0];
frag=document.createDocumentFragment();if(arg instanceof Array)array.each(arg,function(i){element.html(element.create(array.cast(i,true)[0],frag),array.cast(i)[0])});else utility.iterate(arg,function(v,k){if(typeof v==="string")element.html(element.create(k,undefined,frag),v);else if(v instanceof Array||v instanceof Object)utility.tpl(v,element.create(k,undefined,frag))});target.appendChild(frag);return array.last(target.childNodes)},uuid:function(safe){var s=function(){return((1+Math.random())*65536|
0).toString(16).substring(1)},r=[8,9,"a","b"],o;o=s()+s()+"-"+s()+"-4"+s().substr(0,3)+"-"+r[Math.floor(Math.random()*4)]+s().substr(0,3)+"-"+s()+s()+s();if(safe===true)o=o.replace(/-/g,"");return o},walk:function(obj,arg){array.each(arg.replace(/\]$/,"").replace(/\]/g,".").replace(/\.\./g,".").split(/\.|\[/),function(i){obj=obj[i]});return obj},when:function(){var i=0,defer=deferred(),args=array.cast(arguments),nth;if(args[0]instanceof Array)args=args[0];nth=args.length;if(nth===0)defer.resolve(null);
else array.each(args,function(p){p.then(function(){if(++i===nth&&!defer.isResolved())if(args.length>1)defer.resolve(args.map(function(obj){return obj.value||obj.promise.value}));else defer.resolve(args[0].value||args[0].promise.value)},function(){if(!defer.isResolved())if(args.length>1)defer.reject(args.map(function(obj){return obj.value||obj.promise.value}));else defer.reject(args[0].value||args[0].promise.value)})});return defer}};var validate={test:function(args){var exception=false,invalid=[],
value=null,c=[],p;if(args.nodeName!==undefined&&args.nodeName==="FORM"){if(string.isEmpty(args.id))utility.genId(args);c=utility.dom("#"+args.id+" input, #"+args.id+" select");array.each(c,function(i){var z={},p,v,r;p=regex[i.nodeName.toLowerCase()]?regex[i.nodeName.toLowerCase()]:!string.isEmpty(i.id)&&regex[i.id.toLowerCase()]?regex[i.id.toLowerCase()]:"notEmpty";v=element.val(i);if(v===null)v="";z[p]=v;r=validate.test(z);if(!r.pass){invalid.push({element:i,test:p,value:v});exception=true}})}else utility.iterate(args,
function(v,k){if(v===undefined||v===null){invalid.push({test:k,value:v});exception=true;return}value=v.toString().charAt(0)==="#"?utility.dom(v)!==undefined?element.val(utility.dom(v)):"":v;if(k==="date"){if(isNaN((new Date(value)).getYear())){invalid.push({test:k,value:value});exception=true}}else if(k==="domain"){if(!regex.domain.test(value.replace(regex.scheme,""))){invalid.push({test:k,value:value});exception=true}}else if(k==="domainip"){if(!regex.domain.test(value.replace(regex.scheme,""))||
...
node("item",arg);else if(typeof arg==="object")utility.iterate(arg,function(v,k){x+=xml.encode(v,typeof v==="object",false).replace(/item|xml/g,isNaN(k)?k:"item")});x+=wrap?"</xml>":"";if(top)x='<?xml version="1.0" encoding="UTF8"?>'+x;return x}catch(e){utility.error(e,arguments,this);return undefined}},valid:function(arg){return xml.decode(arg).getElementsByTagName("parsererror").length===0}};function Abaaso(arg){var self=this;if(arg)array.each(utility.$(arg),function(i){self.push(i)})}Abaaso.prototype=
[];Abaaso.prototype.constructor=Abaaso;Abaaso.prototype.addClass=function(arg){return array.each(this,function(i){element.klass(i,arg)})};Abaaso.prototype.after=function(type,args){var result=new Abaaso;array.each(this,function(i){result.push(element.create(type,args,i,"after"))});return result};Abaaso.prototype.append=function(type,args){var result=new Abaaso;array.each(this,function(i){result.push(element.create(type,args,i,"last"))});return result};Abaaso.prototype.appendTo=function(obj){var target=
obj[0]||obj;return array.each(this,function(i){target.appendChild(i)})};Abaaso.prototype.at=function(n){var result=new Abaaso;result.push(this[n]);return result};Abaaso.prototype.attr=function(key,value){return array.each(this,function(i){element.attr(i,key,value)})};Abaaso.prototype.before=function(type,args){var result=new Abaaso;array.each(this,function(i){result.push(element.create(type,args,i,"before"))});return result};Abaaso.prototype.clear=function(){return array.each(this,function(i){element.clear(i)})};
...
template,options){return array.each(this,function(i){datalist(i,store,template,options)})};Abaaso.prototype.disable=function(){return array.each(this,function(i){element.disable(i)})};Abaaso.prototype.dispatch=function(event,data,bubbles,cancelable){return array.each(this,function(i){element.dispatch(i,event,data,bubbles,cancelable)})};Abaaso.prototype.destroy=function(){array.each(this,function(i){element.destroy(i)});return new Abaaso};Abaaso.prototype.each=function(arg,async,size){var self=this;
return array.each(this,function(i,idx){var instance=new Abaaso;instance.push(i);arg.call(self,instance,idx)},async,size)};Abaaso.prototype.enable=function(){return array.each(this,function(i){element.enable(i)})};Abaaso.prototype.find=function(arg){var result=new Abaaso;array.each(this,function(i){array.each(element.find(i,arg),function(r){result.push(r)})});return result};Abaaso.prototype.fire=function(){var args=arguments;return array.each(this,function(i){observer.fire.apply(observer,[i].concat(array.cast(args)))})};
Abaaso.prototype.filter=function(datalist,filters,debounce){return array.each(this,function(i){filter(i,datalist,filters,debounce)})};Abaaso.prototype.forEach=function(arg,async,size){return this.each(arg,async,size)};Abaaso.prototype.genId=function(){return array.each(this,function(i){utility.genId(i)})};Abaaso.prototype.get=function(uri,headers){return array.each(this,function(i){client.request("GET",uri,headers,function(arg){element.html(i,arg)},function(e){element.html(i,e)})})};Abaaso.prototype.grid=
function(store,fields,sortable,options,filtered,debounce){return array.each(this,function(i){grid(i,store,fields,sortable,options,filtered,debounce)})};Abaaso.prototype.has=function(arg){var result=new Abaaso;array.each(this.filter(function(i){return element.has(i,arg)}),function(i){result.push(i)});return result};Abaaso.prototype.hasClass=function(arg){var result=new Abaaso;array.each(this.filter(function(i){return element.hasClass(i,arg)}),function(i){result.push(i)});return result};Abaaso.prototype.html=
function(arg){var result;if(arg!==undefined){array.each(this,function(i){element.html(i,arg)});return this}else{result=[];array.each(this,function(i){result.push(element.html(i))});return result}};Abaaso.prototype.is=function(arg){var result=new Abaaso;array.each(this.filter(function(i){return element.is(i,arg)}),function(i){result.push(i)});return result};Abaaso.prototype.isAlphaNum=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isAlphaNum(i)}),function(i){result.push(i)});
return result};Abaaso.prototype.isBoolean=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isBoolean(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isChecked=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isChecked(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isDate=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isDate(i)}),function(i){result.push(i)});
return result};Abaaso.prototype.isDisabled=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isDisabled(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isDomain=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isDomain(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isEmail=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isEmail(i)}),function(i){result.push(i)});
return result};Abaaso.prototype.isEmpty=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isEmpty(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isHidden=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isHidden(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isIP=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isIP(i)}),function(i){result.push(i)});return result};
Abaaso.prototype.isInt=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isInt(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isNumber=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isNumber(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isPhone=function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isPhone(i)}),function(i){result.push(i)});return result};Abaaso.prototype.isUrl=
function(){var result=new Abaaso;array.each(this.filter(function(i){return element.isUrl(i)}),function(i){result.push(i)});return result};Abaaso.prototype.last=function(arg){var result=new Abaaso,tmp=array.last(this,arg);if(isNaN(arg)||arg<2)tmp=tmp!==undefined?[tmp]:[];array.each(tmp,function(i){result.push(i)});return result};Abaaso.prototype.limit=function(start,offset){var result=new Abaaso;array.each(array.limit(this,start,offset),function(i){result.push(i)});return result};Abaaso.prototype.listeners=
function(event){var result=[];array.each(this,function(i){result.push(abaaso.listeners(i,event))});return result};Abaaso.prototype.loading=function(){return array.each(this,function(i){utility.loading(i)})};Abaaso.prototype.on=function(event,listener,id,scope,state){return array.each(this,function(i){observer.add(i,event,listener,id,scope||i,state)})};Abaaso.prototype.once=function(event,listener,id,scope,state){return array.each(this,function(i){observer.once(i,event,listener,id,scope||i,state)})};
Abaaso.prototype.position=function(){var result=[];array.each(this,function(i){result.push(element.position(i))});return result};Abaaso.prototype.prepend=function(type,args){var result=new Abaaso;array.each(this,function(i){result.push(element.create(type,args,i,"first"))});return result};Abaaso.prototype.remove=function(start,end){return array.remove(this,start,end)};Abaaso.prototype.removeIf=function(fn){return array.removeIf(this,fn)};Abaaso.prototype.removeWhile=function(fn){return array.removeWhile(this,
